...
find_acc_outliers <- function(dat, by="subject", condition=2.5){
  acc_outlier <- group_by(dat, rotation, match) %>%
    dplyr::summarise(mean_correct = mean(correct),
              SD_correct = sd(correct),
              lower = mean_correct-condition*SD_correct,
              .groups="keep") %>%
    dplyr::select(rotation, match, lower) %>%
    full_join(dat)
  if (by == "subject"){
    sub_summary <- group_by(acc_outlier, sub, rotation, match) %>%
    ...
    image_summary <- group_by(acc_outlier, imagename, rotation, match) %>%
